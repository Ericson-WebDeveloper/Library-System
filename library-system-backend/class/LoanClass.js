...
class LoanClass {
  ...
  async queryWithAggregate(searchInputs, startIndex = 0, limit = 2) {
    if (!searchInputs) {
      ...
      searchInputs.book == "" &&
      searchInputs.user == "" &&
      searchInputs.issue_date == "" &&
      searchInputs.due_date == "" &&
      searchInputs.return_date == "" &&
      searchInputs.status == ""
    ) {
      // return {$match};
      return await Loan.find()
      ...
      if (searchInputs.book != "") {
        conditions.push({
          "book.title": { $regex: `.*${searchInputs.book}.*`, $options: "i" },
        });
      }
      if (searchInputs.user != "") {
        conditions.push({
          "user.fullname": {
            $regex: `.*${searchInputs.user}.*`,
            $options: "i",
          },
        });
      }
      if (searchInputs.issue_date != "") {
        conditions.push({
          $expr: { $eq: ["$issuedate", searchInputs.issue_date] },
        });
      }
      if (searchInputs.due_date != "") {
        conditions.push({
          $expr: { $eq: ["$duedate", searchInputs.due_date] },
        });
      }
      if (searchInputs.return_date != "") {
        conditions.push({
          $expr: { $eq: ["$returndate", searchInputs.return_date] },
        });
      }
  ...
  async loans(pageNumber, limit, searchInput = null) {
    ...
    result.next =
      endIndex < count ? { pageNumber: pageNumber + 1, limit: limit } : null;
    ...
    return await Loan.aggregate([
      ...
      },
      {
        $project: {
          ...
          Loan_Not_Return: {
            $arrayElemAt: ["$Loan_Not_Return.Total_Loans_Not_Return", 0],
          },
        },
      },
    ]);
  ...
  async loanValidateWarning(id) {
    return await Loan.findById(id).populate({
      path: "user",
      select: "warning",
    });
