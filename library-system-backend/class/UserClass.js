...
class UserCLass {
  ...
  async users(pageNumber, limit, searchInputs = null) {
    ...
    if (searchInputs && searchInputs.fullname != "") {
      conditions.push({
        fullname: { $regex: `.*${searchInputs.fullname}.*`, $options: "i" },
      });
    ...
    if (searchInputs && searchInputs.email != "") {
      conditions.push({
        email: { $regex: `.*${searchInputs.email}.*`, $options: "i" },
      });
    }
  ...
  async createToken(id, token) {
    ...
    return await User.findOneAndUpdate(
      ...
      {
        "token.value": token,
        "token.expires": date.toISOString("en-Us", { timeZone: "Asia/Manila" }),
      },
      {
  ...
  async userResetWarning(id) {
    return await User.findOneAndUpdate({ _id: id }, { $set: { warning: 0 } })
      .select("-password")
      .populate("role");
  }

  async userIncWarning(id) {
    return await User.findOneAndUpdate({ _id: id }, { $inc: { warning: 1 } })
      .select("-password")
      .populate("role");
  }
