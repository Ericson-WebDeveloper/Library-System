...
const RegisterUser = async (req, res) => {
  try {
    ...
    if (!role) {
      return res
        .status(400)
        .json({ message: "Server encounter error. please try again later." });
    }
    if (await UserClass.findByEmailUser(req.body.email)) {
      return res
        .status(400)
        .json({ message: "Email is already use. please use different email." });
    }
    const data = {
...
const checkEmailResetPass = async (req, res) => {
  try {
    ...
    if (userResponse) {
      ...
      let senEmail = await sendEmailResetLink(
        response.email,
      ...
      return res
        .status(200)
        .json({ message: "Request Success. Link was send in your email." });
    }

    return res
      .status(400)
      .json({ message: "Request Failed Email Invalid. Try Again." });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
      .json({
        message: "Reset Password request Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
...
const processResetPass = async (req, res) => {
  try {
    ...
    if (!userResponse.token.value) {
      return res
        .status(400)
        .json({ message: "No Token Generate for this request." });
    }
    ...
    if (minutes >= 5) {
      ...
      return res
        .status(400)
        .json({ message: "Token Expires. sorry you can request again." });
    }
    ...
    if (response) {
      ...
      return res
        .status(200)
        .json({ message: "Password Reset Complete. You can login now." });
    }
...
const RegisterNewUser = async (req, res) => {
  try {
    ...
    let response = await UserClass.register(data);
    let sendemail = await sendEmailRegister(
      req.body.email,
      "librarysystem@gmail.com",
      "User Registration",
      response,
    ...
    if (!sendemail) {
      ...
      return res
        .status(400)
        .json({ message: "Register Failed. Server Email Send Error." });
    }
...
const LoginUser = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({
        message: "Sign In Failed. Server Error.",
        error_details: { ...error },
      });
...
const LogOutUser = (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Logout Failed. Server Error.", error_details: error });
...
const dashBoardDatas = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Request Complete", users, librarians });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
      .json({ message: "Server Error.", error_details: error });
  }
};
...
const ListUsers = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "User List Request Complete", users });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
...
const UpdatingProfileData = async (req, res) => {
  try {
    ...
    const datas = {
      ...
      middlename,
      email,
      password: password == "" ? undefined : await hashPass(password),
      details: {
        phone,
    ...
    if (userResponse) {
      ...
      return res
        .status(200)
        .json({ message: "Profile Update Success", user: userResponse });
    }
    return res
      .status(400)
      .json({ message: "Profile Update Failed.Please Try Again", user });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
...
const UpdatingProfileVatar = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Profile Update Avatar Complete", user });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
...
const UpdatingWarning = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ message: "User Warning Details Update Failed." });
    }
    user = await UserClass.findByEmailUser(user.email);
    return res
      .status(200)
      .json({ message: "User Warning Details Complete", user });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
      .json({
        message: "User Warning Details Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
