...
const RegisterBook = async (req, res) => {
  try {
    ...
    let response = await BookClass.addBook({
      title,
      isbn,
      author,
      categories,
      copies,
      image,
      status: copies >= 1 ? "available" : "unavailable",
    });
    if (response) {
      return res
        .status(200)
        .json({ message: "Register New Book Success", book: response });
    }
    ...
    return res
      .status(500)
      .json({
        message: "Registration New Book Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
...
const BooksAll = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Request Book List Success", books: response });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
      .json({
        message: "Request Book List Failed. Server Error.",
        error_details: Object.values(error),
      });
  }
};
...
const GetBook = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Request Book Success", book: response });
  } catch (error) {
    return res
      .status(500)
      .json({
  ...
  try {
    ...
    if (count >= 1) {
      return res
        .status(400)
        .json({
          message: "Cannot Delete this book. other copies was borrowed.",
        });
    }
    ...
    return res
      .status(500)
      .json({
        message: "Request Book Delete Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
...
const UpdateBook = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, isbn, author, categories, copies, image } = req.body;
    let response = await BookClass.bookUpdate({
      id,
      title,
      isbn,
      author,
    ...
    if (response) {
      return res
        .status(200)
        .json({ message: "Updating Book Success", book: response });
    }
    ...
    return res
      .status(500)
      .json({
        message: "Updating Book Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
...
const BooksDashBoard = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Book DashBoard Request Success", datas });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
      .json({
        message: "Book DashBoard Request Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
