  ...
  try {
    ...
    if (userBorrower && userBorrower.warning >= 3) {
      return res
        ...
        .json({
          message:
            "This User Has many warning. for the mean time this user are not allowed borrowed",
        });
    }
    ...
    if (responseLoan && bookResponse) {
      return res
        .status(200)
        .json({ message: "Create Borrowed Book Success", loan: responseLoan });
    }
    ...
    return res
      .status(500)
      .json({
        message: "Create Borrowed Book Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
...
const LoansList = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Loan List Request Success", loans: response });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
      .json({
        message: "Loan List Request Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
...
const SendEmailNotifLoanReturn = async (req, res) => {
  try {
    ...
    if (!loan) {
      return res
        .status(400)
        .json({ message: "Cannot Find Data. Cannot Send Email Notification" });
    }
    ...
    let response = await sendEmailLoanNotif(
      user.email,
      "librarysystem@gmail.com",
      "Return Book's Notification",
      user,
      book,
      date
    );
    if (!response) {
      return res
        .status(400)
        .json({
          message: "Cannot Send Email Notification. Email Server Error",
        });
    }
  ...
  try {
    ...
    if (isUser) {
      if (response.user._id.equals(_id)) {
        return res
          .status(200)
          .json({ message: "Loan Details Request Success", loan: response });
      }
      return res
        .status(401)
        .json({ message: "You are unAthorized to access this data" });
    }
    return res
      .status(200)
      .json({ message: "Loan Details Request Success", loan: response });
...
const UpdateLoanDueDate = async (req, res) => {
  try {
    ...
    let date = currentDateTime.toISOString("en-US", {
    ...
    if (bookResponse.copies != 0 && bookResponse.status == "unavailable") {
      bookResponse = await BookClass.bookUpdateStatus(
        bookResponse._id,
        "available"
      );
    }
    ...
    if (loanresponse && bookResponse) {
      return res
        .status(200)
        .json({ message: "Loan Update Request Success", loan: loanresponse });
    }
    return res
      .status(200)
      .json({ message: "Loan Update Request Success", loan: loanresponse });
    ...
    return res
      .status(500)
      .json({
        message: "Loan Update Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
...
const LoansListUser = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Loan List By User Request Success", loans });
  } catch (error) {
    return res
      .status(500)
      .json({
        message: "Loan List By User Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
...
const LoanDashBoardDatas = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Loan Counts Request Success", loanscount });
  } catch (error) {
    return res
      .status(500)
      .json({
        message: "Loan Counts Request Failed. Server Error.",
        error_details: { ...error },
      });
  }
};
...
const LoanDashBoardDataUser = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({
        message: "Loan Counts Request Success",
        loancount: loanscount[0],
      });
  } catch (error) {
    return res
      .status(500)
